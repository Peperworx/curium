class Compiler:
    ...
    def resolve_value(self, value, nst):
        if value["type"] == "integer_literal":
            return "",value["value"]
        elif value["type"] == "name":
            print(self.symbol_table)
            out = ""
            ste =  self.symbol_table[value["value"]][-1]
            mname = f'{ste["name"]}.{len(self.symbol_table[value["value"]])-1}'
            if not ste["global"]:
                mname = f"%{mname}"
                if ste["mutable"]:
                    out += f'{nst}%_R{mname} = load {ste["second_type"]}, {ste["second_type"]}* {mname}'
                    out += "\n"
    ...
    def resolve_return(self, in_tree, out = "", parent=None, nesting = 0):
        nst = '    '*nesting
        rv = self.resolve_value(in_tree['arguments'][0],nst)
        out += rv[0]
        out += f"{nst}ret {parent['returns']} {rv[1]}"
        return out
    ...
    def create_global_variable(self, in_tree, nesting = 0):
        ...
        nst = '    '*nesting
        rv = self.resolve_value(in_tree["value"],nst)
        var = rv[0]
        var += f'{nst}@{val["name"]}.{len(self.symbol_table[val["name"]])-1} = global {self.map_type(val["second_type"])} {rv[1]}'
    ...
    def create_local_variable(self, in_tree, nesting = 0):
        ...
        if in_tree["mutable"]:
            ...
            rv = self.resolve_value(in_tree["value"], ident)
            var += rv[0]
            var += f'{ident}store {self.map_type(val["second_type"])} {rv[1]}, {self.map_type(val["second_type"])}* {val["name"]}.{len(self.symbol_table[val["name"]])-1}'
        else:
